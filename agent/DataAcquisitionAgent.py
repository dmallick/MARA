...
class DataAcquisitionAgent:
    ...
    def execute_task(self):
        ...
        print(f"{self.name}: Current task retrieved: {task}")
        if not task or task["type"] != "web_scrape":
            ...
            return
        print(f"{self.name}: Proceeding with web scraping task: {task}")
        ...
        print(f"{self.name}: Using OPENAI_API_KEY: {openai_key is not None}")
        if not openai_key:
            ...
            return
        print(f"{self.name}: Using source URL: {source_url}")
        graph_config = {
           "llm": {
              "api_key": openai_key,
              "model": "openai/gpt-4o",
        ...
        print(f"{self.name}: Graph configuration set for scraping.")
        try:
            ...
            result = scraper.run()
            print(f"{self.name}: Scraping completed successfully. Result: {result[:10]}...")  # Print first 100 chars of result
